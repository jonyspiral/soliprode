...
export function HighlightMetric({ value, label, detail }: HighlightMetricProps) {
  return (
    <article className="rounded-[1.5rem] border border-white/20 bg-white/10 p-4 text-white backdrop-blur">
      <p className="text-2xl font-serif sm:text-3xl">{value}</p>
...
export function ScopeCard({ title, summary, status, detail }: ScopeCardProps) {
  return (
    <article className="rounded-[1.5rem] border border-[var(--color-line)] bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
      ...
      <p className="mt-4 text-sm text-[var(--color-ink)]">{detail}</p>
    </article>
  );
}
